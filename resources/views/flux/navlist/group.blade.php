...
<?php if ($expandable && $heading): ?>
    <ui-disclosure {{ $attributes->class('group/disclosure') }} @if ($expanded === true) open @endif data-flux-navlist-group>
        <button type="button" class="w-full h-10 lg:h-8 flex items-center group/disclosure-button mb-[2px] rounded-lg hover:bg-zinc-800/5 dark:hover:bg-white/[7%] text-white hover:text-white/80 dark:text-white/80 dark:hover:text-white">
            <div class="ps-3 pe-4">
            ...
            <span class="text-sm font-small leading-none">{{ $heading }}</span>
        </button>
...
<?php elseif ($heading): ?>
    <div {{ $attributes->class('block space-y-[2px]') }}>
        <div class="px-3 py-2">
            <div class="text-sm text-white/40 font-small leading-none">{{ $heading }}</div>
        </div>
