<div class="mb-18">
    ...
    <div class="my-3">
        <x-navigations.flat-tab>
            <x-navigations.flat-tab-item href="admin.data_verification.registration_payment.payment_unpaid" label="Belum"
                :isActive="true" activeTextColor="text-white" />
            <x-navigations.flat-tab-item href="admin.data_verification.registration_payment.payment_process"
                label="Proses" />
            <x-navigations.flat-tab-item href="admin.data_verification.registration_payment.payment_paid"
                label="Sudah" />
        </x-navigations.flat-tab>
    </div>
    ...
    <x-animations.fade-down showTiming="50">
        <div class="space-y-2">
            ...
            <div class="flex justify-between mt-3 gap-3">
                ...
                <flux:badge variant="solid" color="primary" icon="user-check">Jumlah : {{ $this->totalStudent }}
                </flux:badge>
            </div>
        </div>
                    ...
                    <x-cards.flat-card
                        ...
                        <x-slot:label>
                            ...
                            <div wire:loading.remove wire:target="fuPayment({{ $student->id }})">
                                <x-items.wa-icon width="25" height="25"
                                    wire:click="fuPayment({{ $student->id }})" />
                            </div>
                        </x-slot:label>

                        <flux:text variant="soft">Tanggal Daftar:
                            <strong
                                class="text-white">{{ \App\Helpers\DateFormatHelper::indoDateTime($student->registration_date) }}</strong>
                        </flux:text>
