<div>
    <flux:modal name="{{ $modalId }}" class="md:w-150" @close="resetAllProperty">
        <form wire:submit='saveOwner'
        x-data="
        formValidation({
            ownerName: ['required', 'minLength:3'],
            mobilePhone: ['required', 'minLength:7', 'maxLength:12'],
            email: ['required'],
            password: ['required', 'minLength:6'],
        })"
        x-on:open-add-edit-owner-modal.window="
        isSubmitActive = true;
        isModalLoading = true;
        isEditingMode = true;
            ...
            <div class="space-y-4" x-show="!isModalLoading">
                <div>
                    <flux:heading size="xl" class="text-center">
                            {{ $isEditing ? "Edit Data Owner" : "Tambah Owner" }}
                    </flux:heading>
