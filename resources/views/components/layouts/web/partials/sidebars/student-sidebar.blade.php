<flux:sidebar class="bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700">
    ...
    <flux:sidebar.nav>
        <flux:sidebar.item
            icon="home"
            href="{{ route('student.student_dashboard') }}"
            :current="Route::is('student.student_dashboard')"
            wire:navigate>
            {{ __('Dashboard') }}
        </flux:sidebar.item>
        ...
        <flux:navlist.group :heading="__('Data Siswa')" class="grid">
            ...
            <flux:sidebar.item
                :current="Route::is('student.payment.registration_payment')"
                icon="banknotes"
                href="{{ route('student.payment.registration_payment') }}"
                wire:navigate>
            ...
            <!--Biodata-->
            <flux:sidebar.item
                :current="Route::is('student.admission_data.biodata')"
                icon="contact-round"
            ...
            <!--Berkas-->
            <flux:sidebar.item
                :current="Route::is('student.admission_data.admission_attachment')"
                icon="file-badge"
        ...
        <flux:navlist.group :heading="__('Tes dan Pengumuman')" class="grid">
            ...
            <flux:sidebar.item
                :current="Route::is('student.placement_test.qr_presence_test')"
                icon="qr-code"
                href="{{ route('student.placement_test.qr_presence_test') }}"
                wire:navigate>
            {{ __('QR Code Tes') }}
            </flux:sidebar.item>
            <!--#Placement Test Presence-->
            ...
            <flux:sidebar.group
                expandable
                :expanded="Route::is('student.placement_test.announcement.*')"
                icon="megaphone"
                heading="{{ __('Pengumuman') }}"
                class="grid">
                    <flux:sidebar.item
                    href="{{ route('student.placement_test.announcement.private_announcement') }}"
                    :current="Route::is('student.placement_test.announcement.private_announcement')"
                    wire:navigate>
                        {{ __('Kelulusan Pribadi') }}
                    </flux:sidebar.item>

                    <flux:sidebar.item
                    href="{{ route('student.placement_test.announcement.public_announcement') }}"
                    :current="Route::is('student.placement_test.announcement.public_announcement')"
                    wire:navigate>
                        {{ __('Kelulusan Umum') }}
                    </flux:sidebar.item>
            </flux:sidebar.group>
